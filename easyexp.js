...
var EASYEXPERIENCE = EASYEXPERIENCE || (function() {
    ...
    var version = '1.11521693362',
        lastUpdate = 1518023682,
    ...
    checkInstall = function() {
        ...
        if(!_.has(state,'EASYEXPERIENCE') || state.EASYEXPERIENCE.version !== schemaVersion){
            state.EASYEXPERIENCE = state.EASYEXPERIENCE || {};
            log('  > Updating Schema to v'+schemaVersion+' <');
            state.EASYEXPERIENCE.version = schemaVersion;
			state.EASYEXPERIENCE.config = state.EASYEXPERIENCE.config || {};
		};
        setDefaults();
        sFail = stateCheck();
        loadSettings();
...
//RECORDING OR APPLYING XP: applyXP, recordXP, characterXP, levelUP
    ...
    levelUP = function(level){
        log('level: '+JSON.stringify(level));
        var thresholds = getThresholds(),
        currXP = findObjs({
            type: 'attribute',
            name: state.EASYEXPERIENCE.xpattr,
        ...
        currLevel = (''+level.get('current')).trim()*1,
        nextLevel;
